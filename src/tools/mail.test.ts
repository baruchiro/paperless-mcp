...
test("create_mail_rule forwards an explicit null filter so it can be cleared (#138)", async () => {
  const calls: Record<string, unknown>[] = [];
  const api = {
    createMailRule: async (data: Record<string, unknown>) => {
      calls.push(data);
      return { id: 1, ...data };
    },
  } as unknown as PaperlessAPI;
  ...
  assert.equal(calls[0].filter_from, null);
});

test("create_mail_rule rejects a filter longer than the spec's 256 characters (#138)", async () => {
  const calls: Record<string, unknown>[] = [];
  const api = {
    createMailRule: async (data: Record<string, unknown>) => {
      calls.push(data);
    ...
    try {
      result = (await client.callTool({
        ...
        arguments: {
          ...
          account: 1,
          folder: "INBOX",
          filter_from: "x".repeat(257),
        },
      })) as CallToolResult;
    } catch {
      rejected = true;
    }
    assert.ok(rejected || result?.isError, "expected a 257-character filter to be rejected");
  });
